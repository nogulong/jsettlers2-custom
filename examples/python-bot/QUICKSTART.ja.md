...
JSettlersでは、各ターンは以下の流れで進行します：

1. **サイコロを振る（自動）** - ゲーム状態15で自動的に実行
2. **アクションフェーズ** - ゲーム状態20で建設や交易などを実行
3. **ターン終了** - `ENDTURN`メッセージを送信

### ボットが自動的に処理する状況

ボットは以下の状況を自動的に処理します：

- **カードを捨てる**: 7が出て資源が8枚以上の場合、ランダムに捨てる
- **トレードの提案**: 他のプレイヤーからのトレード提案は常に断る
- **盗賊で奪う相手**: 選択肢がある場合は最初のプレイヤーを選択
